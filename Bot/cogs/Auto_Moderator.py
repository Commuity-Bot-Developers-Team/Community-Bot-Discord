...
class Auto_Moderator(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        for rule_name in RULE_FUNCTION_MAPPING:
            rule_config = {'max': 200, 'interval': 15, 'punishment': 'mute', 'time': None}
            ...
            if result is not None:
                ...
                if not (rule_config['punishment'] == "kick" or rule_config['punishment'] == "voice-kick"):
                    ...
                    await command_for_punishment(message, message.guild.me, members, full_reason, time=rule_config['time'], reply=False)

                await self.moderation_commands.mute_function(message, message.guild.me, members, full_reason, time=rule_config['time'], reply=False)
                break
