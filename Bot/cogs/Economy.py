...
class Economy(commands.Cog):
                ...
                if die == 3:
                    ...
                    while i:
                        ...
                        if msgco == fthingy:
                            ...
                            await ctx.send(f"You escaped! But lost ``{coinremove}`` from your wallet! :(")
                            await self.bot.pg_conn.execute(
                                "UPDATE users_data SET coins = $1 WHERE userid = $2", coins, user)
    ...
    async def work(self,ctx):
        ...
        rnum = randint(0, 3000)
        minusjobs = 250
        jobs = [f"You worked as a software developer and earned: `{rnum}` coins", f"You worked as a dentist and accidentally pulled out a tooth and earned `{rnum}` coins ", f"You worked as a professional sleeper and earned `{rnum}` coins", f"You spent 8 hours filling seats and earned: `{rnum} coins` ", f"You worked as a surfing Instructor and found your hidden talent with {rnum} coins!", ]
